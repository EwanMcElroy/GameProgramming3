...
void Camera::relativeRotation(Entity* target)
{
	m_viewMatrix = glm::lookAt(target->GetTransform()->GetPosition(), target->GetTransform()->GetPosition() + target->GetTransform()->GetForward(), target->GetTransform()->GetUp());
	VP = m_projMatrix * m_viewMatrix;
}